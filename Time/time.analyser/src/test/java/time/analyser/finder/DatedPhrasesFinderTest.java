...
public class DatedPhrasesFinderTest {
	...
	public void testMillion9() {
		assertOnly(Finder.MILLIARD, milliardYearIs(4.5d), "Par exemple, pour savoir comment était l’Univers au moment de la naissance du Soleil, il y a 4,5 milliards d’années, il suffit d’observer des astres qui sont à 5 milliards d’années-lumière de nous.");
	}
    ...
    private final void assertTwo(final Finder finderName, Condition<? super DatedPhrase> condition1, final Finder finder2Name, Condition<? super DatedPhrase> condition2, String phrase) {
        final DatedPhrasesFinder finder1 = datedPhrasesFinders.get(finderName);
        final List<DatedPhrase> actualPhrases = finder1.findPhrases(phrase);
        assertThat(actualPhrases).as("doit trouver une date").hasSize(1);
        assertThat(actualPhrases.get(0)).as("première date").has(condition1);

        final DatedPhrasesFinder finder2 = datedPhrasesFinders.get(finder2Name);
        final List<DatedPhrase> actualPhrases2 = finder2.findPhrases(phrase);
        assertThat(actualPhrases2).as("doit trouver une date").hasSize(1);
        assertThat(actualPhrases2.get(0)).as("deuxième date").has(condition2);


        final DatedPhrasesFinder[] filteredFinders = Arrays.stream(finders).filter(f -> (f != finder1 && f != finder2)).toArray(size -> new DatedPhrasesFinder[size]);
        assertNoDateIn(phrase, filteredFinders);
    }
