...
public class DateFinderConfig {
    ...
    public DateFinder jcFinder() {
        final Pattern pattern = Pattern.compile("(^| |,)([Aà] partir de|[Ee]n) (l'an )?(?<g>(-)?\\d{2,4})(,? )(ans )?(?<neg>(avant|av.) J.-?C.)?");
    ...
    public DateFinder nearJcFinder() {
        final Pattern pattern = Pattern.compile("(^| |,)([Ee]nviron) (?<g>(-)?\\d{2,4})(,? )ans (?<neg>(avant|av.) J.-?C.)?");
        final IParser parser = new JCParser();
    ...
    public DateFinder nearJcFinder2() {
        final Pattern pattern = Pattern.compile("(^| |,)([Vv]ers l'an) (?<g>(-)?\\d{2,4})(,?) (?<neg>(avant|av.) J.-?C.)?");
        final IParser parser = new JCParser();
    ...
    public DateFinder[] finders() {
        return new DateFinder[] { milliardFinder(), jcFinder(),nearJcFinder(),nearJcFinder(), romanFinder(), annee2DotFinder(), preciseFinder() };
    }
}
